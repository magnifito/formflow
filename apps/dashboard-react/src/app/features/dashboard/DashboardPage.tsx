...
export function DashboardPage() {
    ...
    const getSubmitUrl = (form: Form) => `${import.meta.env.VITE_API_BASE_URL || "http://localhost:3000"}/submit/${form.submitHash}`;

    const copyUrl = (hash: string) => {
        navigator.clipboard.writeText(`${import.meta.env.VITE_API_BASE_URL || "http://localhost:3000"}/submit/${hash}`);
    };
