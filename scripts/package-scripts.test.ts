/**
 * Test suite for all package.json scripts
 *
 * This suite validates that:
 * - All scripts exist and are properly defined
 * - Scripts can be parsed and validated
 * - Scripts follow expected patterns
 * - No critical syntax errors in script definitions
 */
...
describe('Package Scripts Test Suite', () => {
  ...
  describe('Command Discovery', () => {
    ...
    it('should list all available commands', () => {
      ...
      console.log(`\nFound ${commandNames.length} scripts:`);
  ...
  describe('Command Execution Validation', () => {
    // These tests verify commands can be parsed by pnpm/node
    // They don't actually execute the commands, just validate syntax

    it('should be able to list all commands via pnpm run', () => {
      try {
        const workspaceRoot = getWorkspaceRoot();
        const output = execSync('pnpm run', {
          encoding: 'utf-8',
          cwd: workspaceRoot,
          stdio: 'pipe'
        });
        ...
        // pnpm run without arguments shows help, which exits with code 1
  ...
  describe('Command Categorization', () => {
    it('should categorize all commands correctly', () => {
      ...
      // Allow for 'nx' command which is a utility, and 'sync-deps' which is a setup utility
      const allowedUncategorized = ['nx', 'sync-deps', 'nuke'];
  ...
  describe('Command Patterns', () => {
    it('should follow consistent naming patterns for test commands', () => {
      ...
      const excludedCommands = ['test:env', 'test:e2e', 'test:apis', 'test-lab', 'test-lab:webhooks'];
